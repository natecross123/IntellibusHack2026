    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={showCentered ? "flex min-h-[calc(100vh-10rem)] flex-col items-center justify-center pb-24 px-2" : "space-y-4 pb-24"}>
      {showCentered ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">Upload audio or paste a URL to detect AI manipulation</p>
          <Tabs defaultValue="upload" className="w-full">
            ...
            <TabsContent value="url">
              <div className="glass-card rounded-2xl p-4">
                <UrlInput placeholder="Paste an audio URL, SoundCloud link, or podcast…" onSubmit={handleUrlScan} loading={scanning} compact />
              </div>
            </TabsContent>
          </Tabs>
        </motion.div>
        ...
        <>
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {scannedUrl ? <>Scanned: <span className="text-primary truncate block max-w-[250px]">{scannedUrl}</span></> : "Detect synthetic voice patterns"}
            </p>
          </div>
